...
class TestPlannerStartup(unittest.TestCase):
    def test_process_starts_and_exposes_configured_parameters(self, proc_info, planner):
        proc_info.assertWaitForStartup(process=planner, timeout=15)
        rclpy.init()
        node = rclpy.create_node("planner_startup_parameter_probe")
        try:
            ...
            self.assertEqual(values[0].integer_value, 3)
        finally:
            node.destroy_node()
            rclpy.shutdown()
