...
PolynomialTraj PolynomialTraj::piecewiseLinearTraj(const Eigen::MatrixXd &Pos, double max_vel)
{
  if (Pos.rows() != 3 || Pos.cols() < 2 || !Pos.allFinite() || !std::isfinite(max_vel) || max_vel <= 0.0)
    throw std::invalid_argument("piecewiseLinearTraj requires finite 3D points and positive max_vel");

  PolynomialTraj traj;
  for (int i = 0; i + 1 < Pos.cols(); ++i)
  {
    ...
    const double duration = length / max_vel;
    const Eigen::Vector3d velocity = delta / duration;
    std::vector<double> cx{0.0, 0.0, 0.0, 0.0, velocity.x(), Pos(0, i)};
    std::vector<double> cy{0.0, 0.0, 0.0, 0.0, velocity.y(), Pos(1, i)};
    std::vector<double> cz{0.0, 0.0, 0.0, 0.0, velocity.z(), Pos(2, i)};
