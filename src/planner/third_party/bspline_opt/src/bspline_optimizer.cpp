...
namespace scan_planner
{
  namespace
  {
    double collisionCheckTimeStep(UniformBspline &traj, const GridMap::Ptr &grid_map)
    {
      return traj.getCollisionCheckTimeStep(grid_map->getResolution());
    }
  } // namespace
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    {
      const double segment_length = (init_points.col(i) - init_points.col(i - 1)).norm();
      const double step_size = segment_length > 1e-6
                                   ? std::min(1.0, grid_map_->getResolution() / segment_length)
                                   : 1.0;
    ...
    if (flag_got_start && !flag_got_end)
    {
      out_id = std::min(i_end, static_cast<int>(init_points.cols()) - 1);
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        double t_step = collisionCheckTimeStep(traj, grid_map_);
        for (double t = 0.0; t <= duration + 1e-6; t += t_step)
        {
          const double tc = std::min(t, duration);
          Eigen::Vector3d pos = traj.evaluateDeBoorT(tc);
          Eigen::Vector3d pos_next = traj.evaluateDeBoorT(std::min(t + t_step, duration));
