...
namespace scan_planner
{
  ...
  {
    ratio = 1.0;
    if (p_ < 2 || control_points_.cols() < p_ + 1 ||
        u_.size() != control_points_.cols() + p_ + 1 ||
        !control_points_.allFinite() || !u_.allFinite() ||
        !std::isfinite(limit_vel_) || limit_vel_ <= 0.0 ||
        !std::isfinite(limit_acc_) || limit_acc_ <= 0.0 ||
        !std::isfinite(feasibility_tolerance_) || feasibility_tolerance_ < 0.0)
      return false;

    if ((u_.tail(u_.size() - 1) - u_.head(u_.size() - 1)).minCoeff() < 0.0 ||
        getTimeSum() <= 0.0)
      return false;

    const Eigen::MatrixXd velocity_points = getDerivativeControlPoints();
    const Eigen::MatrixXd acceleration_points = getDerivative().getDerivativeControlPoints();
    if (!velocity_points.allFinite() || !acceleration_points.allFinite())
      return false;

    const double max_vel = velocity_points.colwise().norm().maxCoeff();
    const double max_acc = acceleration_points.colwise().norm().maxCoeff();
    ratio = std::max({1.0, max_vel / limit_vel_, std::sqrt(max_acc / limit_acc_)});
    const bool feasible = max_vel <= limit_vel_ * (1.0 + feasibility_tolerance_) + 1e-6 &&
                          max_acc <= limit_acc_ * (1.0 + feasibility_tolerance_) + 1e-6;
    if (show && !feasible)
      std::cout << "[Check]: velocity/acceleration bounds " << max_vel << "/" << max_acc << std::endl;
    return feasible;
  }
                                             ...
                                             Eigen::MatrixXd &ctrl_pts)
  {
    ...
    // Least squares only approximates boundary rows. Pin the six boundary
    // controls so subsequent optimization preserves the requested endpoint states.
    for (int end = 0; end < 2; ++end)
    {
      const int offset = end == 0 ? 0 : K - 1;
      const Eigen::Vector3d &position = end == 0 ? point_set.front() : point_set.back();
      const Eigen::Vector3d &velocity = start_end_derivative[end];
      const Eigen::Vector3d &acceleration = start_end_derivative[end + 2];
      ctrl_pts.col(offset) = position - ts * velocity + ts * ts / 3.0 * acceleration;
      ctrl_pts.col(offset + 1) = position - ts * ts / 6.0 * acceleration;
      ctrl_pts.col(offset + 2) = position + ts * velocity + ts * ts / 3.0 * acceleration;
    }
  ...
  double UniformBspline::getCollisionCheckTimeStep(const double &resolution, const double &max_time_step)
  {
    const double safe_resolution = std::max(resolution, 1e-3);
    const double safe_max_step = std::max(max_time_step, 1e-4);

    if (p_ <= 0 || control_points_.cols() < 2)
      return safe_max_step;

    double max_speed = 0.0;
    const Eigen::MatrixXd velocity_points = getDerivativeControlPoints();
    for (int i = 0; i < velocity_points.cols(); ++i)
      max_speed = std::max(max_speed, velocity_points.col(i).norm());

    if (!std::isfinite(max_speed) || max_speed <= 1e-6)
      return safe_max_step;

    return std::max(1e-4, std::min(safe_max_step, safe_resolution / max_speed));
  }
