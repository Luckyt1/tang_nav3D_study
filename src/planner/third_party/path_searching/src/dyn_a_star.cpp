...
ASTAR_RET AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt)
{
    ...
    std::priority_queue<NodeComparator::OpenSetNode, std::vector<NodeComparator::OpenSetNode>, NodeComparator> empty;
    openSet_.swap(empty);
    ...
    openSet_.push(makeOpenSetNode(startPtr)); //put start in open set
    ...
    {
        num_iter++;
        const auto open_node = openSet_.top();
        openSet_.pop();
        if (!isCurrentOpenSetNode(open_node))
            continue;
        current = open_node.node;
        ...
        for (int dx = -1; dx <= 1; dx++)
            for (int dy = -1; dy <= 1; dy++)
            {
                ...
                if (!flag_explored)
                {
                    ...
                    openSet_.push(makeOpenSetNode(neighborPtr)); //put neighbor in open set and record it.
                }
                ...
                { //in open set and need update
                    ...
                    neighborPtr->gScore = tentative_gScore;
                    neighborPtr->fScore = tentative_gScore + getHeu(neighborPtr, endPtr);
                    openSet_.push(makeOpenSetNode(neighborPtr));
                }
            }
