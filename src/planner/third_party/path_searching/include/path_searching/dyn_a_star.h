...
public:
	struct OpenSetNode
	{
		GridNodePtr node{NULL};
		double gScore{inf};
		double fScore{inf};
	};

	bool operator()(const OpenSetNode &node1, const OpenSetNode &node2) const
	{
		return node1.fScore > node2.fScore;
	}
};

inline NodeComparator::OpenSetNode makeOpenSetNode(GridNodePtr node)
{
	return {node, node->gScore, node->fScore};
}

inline bool isCurrentOpenSetNode(const NodeComparator::OpenSetNode &entry)
{
	return entry.node != NULL &&
	       entry.node->state == GridNode::OPENSET &&
	       entry.node->gScore == entry.gScore &&
	       entry.node->fScore == entry.fScore;
}
...
private:
	...
	std::priority_queue<NodeComparator::OpenSetNode, std::vector<NodeComparator::OpenSetNode>, NodeComparator> openSet_;
