...
void GridMap::initMap(rclcpp::Node *node)
{
  ...
  load_parameter(node_, "grid_map.dynamic_obstacle_timeout_s", mp_.dynamic_obstacle_timeout_s_, 0.0);
  if (mp_.dynamic_obstacle_timeout_s_ > 0.0)
    RCLCPP_WARN(node_->get_logger(),
                "grid_map.dynamic_obstacle_timeout_s is ignored: occupancy requires free-ray evidence to clear");
  load_parameter(node_, "grid_map.observation_timeout_s", observation_timeout_s_, 1.0);
  if (!std::isfinite(observation_timeout_s_) || observation_timeout_s_ <= 0.0)
    throw std::invalid_argument("grid_map.observation_timeout_s must be positive and finite");

  load_parameter(node_, "grid_map.vis_height", mp_.vis_height_, 0.3);
  load_parameter(node_, "grid_map.inflate_vis_use_z_slice", mp_.inflate_vis_use_z_slice_, false);
  load_parameter(node_, "grid_map.inflate_vis_z_min", mp_.inflate_vis_z_min_, 0.0);
  load_parameter(node_, "grid_map.inflate_vis_z_max", mp_.inflate_vis_z_max_, 0.8);
  load_parameter(node_, "grid_map.show_occ_time", mp_.show_occ_time_, false);
  load_parameter(node_, "grid_map.occ_interval_ms", mp_.occ_interval_ms_, 125);
  load_parameter(node_, "grid_map.vis_interval_ms", mp_.vis_interval_ms_, 250);
  ...
  load_parameter(node_, "grid_map.ground_filter_height", mp_.ground_filter_height_, 0.0);
  ...
  load_parameter(node_, "grid_map.self_filter_radius", mp_.self_filter_radius_, 0.0);
  load_parameter(node_, "grid_map.cloud_skip_points", mp_.cloud_skip_points_, 1);
  ...
  // Depth resolution is selected by the camera driver at runtime. The point
  // buffer is resized from the actual image dimensions in projectDepthImage().
  ...
  occ_timer_ = node_->create_wall_timer(std::chrono::milliseconds(std::max(50, mp_.occ_interval_ms_)),
                                        std::bind(&GridMap::updateOccupancyCallback, this));
  vis_timer_ = node_->create_wall_timer(std::chrono::milliseconds(std::max(50, mp_.vis_interval_ms_)),
...
void GridMap::applyOccupancyUpdate(const Eigen::Vector3i& id, double new_log_odds)
{
  ...
  Eigen::Vector3d pos;
  indexToPos(id, pos);
  const double ground_filter_top = mp_.ground_height_ + std::max(0.0, mp_.ground_filter_height_);
  if (pos(2) <= ground_filter_top && new_log_odds > mp_.min_occupancy_log_)
    new_log_odds = mp_.clamp_min_log_;
...
void GridMap::updateSlidingMap(const Eigen::Vector3d& center)
{
  ...
  mp_.map_origin_idx_ = new_origin_idx;
  updateMapBoundaryFromIndex();
  // The ring buffer keeps occupancy cells at their local addresses while the
  // global index origin moves.  Rebuild the derived inflation layer after the
  // remap so every retained obstacle is inflated at its new global location.
  rebuildInflation();
...
void GridMap::projectDepthImage()
{
  ...
  const int stride = std::max(1, mp_.skip_pixel_);
  const int margin = std::max(0, mp_.depth_filter_margin_);
  const int usable_cols = cols - 2 * margin;
  const int usable_rows = rows - 2 * margin;
  if (usable_cols <= 0 || usable_rows <= 0)
    return;

  const size_t sampled_cols =
      static_cast<size_t>((usable_cols + stride - 1) / stride);
  const size_t sampled_rows =
      static_cast<size_t>((usable_rows + stride - 1) / stride);
  const size_t required_capacity = sampled_cols * sampled_rows;
  if (md_.proj_points_.size() < required_capacity)
    md_.proj_points_.resize(required_capacity);
  ...
  if (!md_.has_first_depth_)
  {
    ...
    Eigen::Vector3d min_pos = md_.ray_pos_ - mp_.local_update_range_;
    Eigen::Vector3d max_pos = md_.ray_pos_ + mp_.local_update_range_;
    posToIndex(min_pos, md_.local_bound_min_);
    posToIndex(max_pos, md_.local_bound_max_);
    boundIndex(md_.local_bound_min_);
    boundIndex(md_.local_bound_max_);
    return;
  }
  ...
  for (int v = margin; v < rows - margin; v += stride)
  {
    row_ptr = md_.depth_image_.ptr<uint16_t>(v) + margin;

    for (int u = margin; u < cols - margin; u += stride)
    {
...
void GridMap::raycastProcess()
{
  ...
  for (int i = 0; i < md_.proj_points_cnt; ++i)
  {
    ...
    else
    {
      ...
      {
        const bool ground_endpoint =
            pt_w(2) <= mp_.ground_height_ + std::max(0.0, mp_.ground_filter_height_);
        vox_idx = setCacheOccupancy(pt_w, ground_endpoint ? 0 : 1);
      }
    }
...
void GridMap::visCallback()
{
  ...
  if (md_.has_first_depth_ || md_.has_cloud_)
  {
    publishMap();
...
bool GridMap::hasRecentObservation() const
{
  return last_observation_ != std::chrono::steady_clock::time_point{} &&
         std::chrono::duration<double>(std::chrono::steady_clock::now() - last_observation_).count() <=
             observation_timeout_s_;
}
...
void GridMap::updateOccupancyCallback()
{
  ...
  if (!md_.use_cloud_update_)
    projectDepthImage();
  const auto t2 = node_->now();

  raycastProcess();
  if (md_.proj_points_cnt > 0)
    last_observation_ = std::chrono::steady_clock::now();
  const auto t3 = node_->now();
  ...
  if (mp_.show_occ_time_)
  {
    RCLCPP_INFO_THROTTLE(
        ...
        "[GridMap] occupancy update: project=%.1f ms raycast=%.1f ms total=%.1f ms points=%d",
        (t2 - t1).seconds() * 1000.0, (t3 - t2).seconds() * 1000.0,
        (t3 - t1).seconds() * 1000.0, md_.proj_points_cnt);
  }
...
void GridMap::cloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &img)
{
  ...
  const size_t point_step = static_cast<size_t>(std::max(1, mp_.cloud_skip_points_));
  const double self_filter_radius_sq = std::pow(std::max(0.0, mp_.self_filter_radius_), 2);
  for (size_t i = 0; i < latest_cloud.points.size(); i += point_step)
  {
    ...
    if (devi.head<2>().squaredNorm() < self_filter_radius_sq)
      continue;
  ...
  for (int x = min_cut(0); x <= max_cut(0); ++x)
    for (int y = min_cut(1); y <= max_cut(1); ++y)
      for (int z = min_cut(2); z <= max_cut(2); ++z)
      {
        ...
        Eigen::Vector3d pos;
        indexToPos(Eigen::Vector3i(x, y, z), pos);
        if (mp_.inflate_vis_use_z_slice_ &&
            (pos(2) < mp_.inflate_vis_z_min_ || pos(2) > mp_.inflate_vis_z_max_))
          continue;
