...
namespace scan_planner
{
  namespace
  {
    double estimateSegmentYaw(const Eigen::Vector3d &from, const Eigen::Vector3d &to)
    {
      const Eigen::Vector2d diff(to(0) - from(0), to(1) - from(1));
      if (diff.norm() < 1e-6)
        return 0.0;
  ...
  void SCANPlannerManager::initPlanModules(rclcpp::Node *node, PlanningVisualization::Ptr vis)
  {
    ...
    if (!std::isfinite(pp_.max_vel_) || pp_.max_vel_ <= 0.0 ||
      ...
      throw std::invalid_argument("manager.max_vel and manager.max_acc must be positive and finite");
    ...
    const int astar_xy_cells = std::max(
        100, static_cast<int>(std::ceil((pp_.planning_horizon_ + 2.0) / grid_map_->getResolution())));
    bspline_optimizer_rebound_->a_star_->initGridMap(
        grid_map_, Eigen::Vector3i(astar_xy_cells, astar_xy_cells, 30));
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    if (!grid_map_ || !grid_map_->hasRecentObservation() || !start_pt.allFinite() ||
        grid_map_->getInflateOccupancy(start_pt, estimateSegmentYaw(start_pt, local_target_pt)) != 0)
    {
      RCLCPP_WARN(node_->get_logger(), "Cannot plan from an occupied start or without fresh map observations");
      return false;
    }
    ...
    do
    {
      ...
      if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
      {
        ...
        bool flag_too_far;
        ts *= 1.5; // ts will be divided by 1.5 in the next iteration.
        do
        {
          const int segments = std::max(6, static_cast<int>(std::ceil(time / (ts / 1.5))));
          ts = time / segments;
          point_set.clear();
          flag_too_far = false;
          ...
          {
            // The final position and its derivatives must describe the same instant.
            Eigen::Vector3d pt = i == segments ? local_target_pt : gl_traj.evaluate(i * ts);
    ...
    pos.setPhysicalLimits(pp_.max_vel_, pp_.max_acc_, 0.0);

    double ratio = 1.0;
    bool flag_step_2_success = true;
    // Refitting can reintroduce a limit violation; recheck each bounded retry.
    for (int attempt = 0; attempt < 5; ++attempt)
    {
      ...
      flag_step_2_success = refineTrajAlgo(pos, start_end_derivatives, ratio * 1.1, ts, optimal_control_points);
      if (!flag_step_2_success)
        break;
      pos = UniformBspline(optimal_control_points, 3, ts);
    }

    if (!flag_step_2_success || !checkDynamicFeasibility(pos) || !checkTrajectoryCollisionFree(pos))
    {
                                                  ...
                                                  const std::vector<Eigen::Vector3d> &waypoints, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    (void)end_vel;
    (void)end_acc;
    Eigen::MatrixXd pos(3, waypoints.size() + 1);
    pos.col(0) = start_pos;
    for (size_t i = 0; i < waypoints.size(); ++i)
      pos.col(i + 1) = waypoints[i];

    // The global route is a collision-free reference, not the executed trajectory.
    // Piecewise linear segments preserve its corridor and cannot overshoot dense waypoints.
    PolynomialTraj gl_traj = PolynomialTraj::piecewiseLinearTraj(pos, pp_.max_vel_);
  ...
  {
    // Derivative control-point norms bound the entire spline, including between samples.
    position_traj.setPhysicalLimits(pp_.max_vel_, pp_.max_acc_, 0.0);
    double ratio = 1.0;
    return position_traj.checkFeasibility(ratio, false);
  }

  bool SCANPlannerManager::checkTrajectoryCollisionFree(UniformBspline position_traj)
  {
    if (!grid_map_ || !grid_map_->hasRecentObservation())
      return false;
    ...
    const double sample_dt = position_traj.getCollisionCheckTimeStep(grid_map_->getResolution());
    ...
    for (double t = 0.0; t <= duration + 1e-6; t += sample_dt)
    {
      ...
      const Eigen::Vector3d pos_next = position_traj.evaluateDeBoorT(std::min(tc + sample_dt, duration));

      if (grid_map_->getInflateOccupancy(pos, estimateSegmentYaw(pos, pos_next)))
      {
        RCLCPP_WARN(node_->get_logger(),
                    "Trajectory collision check failed: t=%.3f pos=[%.2f, %.2f, %.2f]",
                    tc, pos(0), pos(1), pos(2));
        return false;
      }
    }
                                         ...
                                         Eigen::MatrixXd &ctrl_pts, double &dt, double &time_inc)
  {
    ...
    for (int i = 0; i <= seg_num; ++i)
      point_set.push_back(bspline.evaluateDeBoorT(i == seg_num ? duration : i * dt));
