...
void publishRgb(capture_Image_List_t *stream) {
    if (!stream || stream->imageCount == 0) {
        return;
    }

    buffer_List_t &image = stream->imageList[0];

    if (!image.pAddr || image.length == 0) {
        return;
    }

    cv::Mat decoded_image;
    std::vector<uint8_t> raw_data;
    const auto *payload = static_cast<const uint8_t *>(image.pAddr);
    const bool is_jpeg = image.length >= 3 &&
    ...
    if (!is_jpeg) {
        ...
        const uint64_t nv12_size =
            pixel_count <= std::numeric_limits<uint64_t>::max() - pixel_count / 2
                ? pixel_count + pixel_count / 2
                : std::numeric_limits<uint64_t>::max();
        const uint64_t nv12_rows = static_cast<uint64_t>(image.height) + image.height / 2;
        if (image.width == 0 || image.height == 0 ||
            image.width % 2 != 0 || image.height % 2 != 0 ||
            image.width > static_cast<uint32_t>(std::numeric_limits<int>::max()) ||
            nv12_rows > static_cast<uint64_t>(std::numeric_limits<int>::max()) ||
            nv12_size > image.length) {
            #ifdef ROS2
                RCLCPP_WARN(rclcpp::get_logger("publishRgb"),
                            "Rejecting invalid NV12 frame: format=%u, %ux%u, length=%u, required=%llu",
                            image.format, image.width, image.height, image.length,
                            static_cast<unsigned long long>(nv12_size));
            #else
                ROS_WARN("Rejecting invalid NV12 frame: format=%u, %ux%u, length=%u, required=%llu",
                         image.format, image.width, image.height, image.length,
                         static_cast<unsigned long long>(nv12_size));
            #endif
            return;
        ...
        raw_data.assign(payload, payload + static_cast<size_t>(nv12_size));
        cv::Mat nv12_mat(static_cast<int>(nv12_rows), static_cast<int>(image.width),
                         CV_8UC1, raw_data.data());
        cv::cvtColor(nv12_mat, decoded_image, cv::COLOR_YUV2BGR_NV12);
    } else {
        // MJPEG format: decode JPEG data to bgr8
        raw_data.assign(payload, payload + image.length);
